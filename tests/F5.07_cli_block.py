...
The test runs `text -> cli -> display` in an isolated server. It checks `@in`
substitution, the quoting of values containing spaces, several outputs, and the
presence of stderr in the block logs.
"""
...
def run_cli_case(runtime_mode: str) -> None:
    with isolated_server() as server:
        ...
        expect(run.get("status") == "success", f"The CLI {runtime_mode} run must succeed.")
        ...
        expect("value:hello cli value" in str(run.get("worker_rows", {}).get("display-1", {}).get("received") or ""), "Display does not receive the CLI stdout.")
...
def test_cli_modal_command_first_layout() -> None:
    ...
    expect("data-cli-commands-list" in html, "The CLI modal must show the per-output command list.")
    expect('data-cli-command-port-id="1"' in html, "The CLI modal must show the command of output 1.")
    expect('data-block-output-field="command"' in html, "The CLI command must stay bound to the output.command field.")
